...
# создание юзеров с добавлением в конце ко всем числа
async def create_users(count: int):
    ...
    for i in range(count):
        ...
        # await. Он говорит интерпретатору примерно следующее: "я тут возможно немного потуплю,
        # но ты меня не жди — пусть выполняется другой код, а когда у меня будет настроение продолжиться, я тебе маякну"
    return {'message': f'{count} fake users create'}
    # execute функция базы данны для выполнения команды( т.е говорит выполни то, что сверху query


@app.get('fake_products/{count}')
async def create_products(count: int):
    for i in range(count):
        query = db.users.insert().values(title=f'product {i}', description=f'about product{i}',
...
# response_modelполучает тот же тип, который вы бы объявили для поля модели Pydantic,
# поэтому это может быть модель Pydantic, но также может быть, например, listмодель Pydantic, например List[Item]
...
async def read_user(user_id: int):
    ...
    # ИЗ базы данных колонка c
    user = await db.database.fetch_one(query)
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
        # если такого персонажа нет,выкидываем ошибку, иначе возвращаем его
    return user
